...
Hello their (^ _^)/
    here are a couple notes about my submission
        - if you want to see what the algorithms are doing at each stage i.e.
        see how it's filtering out the bounding boxes. Hop inside the Image.py
        file and change debug from False to True
        ...
        kNN_classfie, and kNN_lables so the algorithm can create new versions
        of these files
"
